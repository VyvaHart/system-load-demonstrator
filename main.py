...
app = Flask(__name__)
print(f"DEBUG_MAIN_PY: Flask app object created: {app}")

metrics = PrometheusMetrics(app)
print(f"DEBUG_MAIN_PY: PrometheusMetrics(app) called directly.")
metrics.info('app_info_direct', 'System Load Application - Direct Init', version='1.0.2-direct')
print(f"DEBUG_MAIN_PY: app.url_map after metrics init: {str(app.url_map)}")

MY_CUSTOM_REQUESTS = Counter('my_custom_requests_v5', 'Total custom requests v5', ['path'])
...
def index():
    ...
    print("DEBUG_MAIN_PY: Route / hit")
    return render_template('index.html')

@app.route('/load', methods=['GET'])
def generate_load():
    MY_CUSTOM_REQUESTS.labels(path="/load").inc()
    print("DEBUG_MAIN_PY: Route /load hit")
...
def manual_metrics_test():
    print("DEBUG_MAIN_PY: /manual_metrics_test endpoint hit!")
    try:
