...
MY_CUSTOM_REQUESTS = Counter('my_custom_requests_total_v3', 'Total custom requests v3', ['path'])
...
def create_app():
    app = Flask(__name__)
    print(f"FACTORY_DEBUG: Flask app object created by factory: {app}")

    @app.route('/')
    def index():
        MY_CUSTOM_REQUESTS.labels(path="/").inc()
        print(f"FACTORY_DEBUG: Route / hit")
        return render_template('index.html')

    @app.route('/load', methods=['GET'])
    def generate_load():
        MY_CUSTOM_REQUESTS.labels(path="/load").inc()
        print(f"FACTORY_DEBUG: Route /load hit")
    ...
    def manual_metrics_test():
        ...
        try:
            ...
            print(f"FACTORY_DEBUG: Generated metrics output length from REGISTRY: {len(output)}")
            return Response(output, mimetype=CONTENT_TYPE_LATEST)
        except Exception as e:
            print(f"FACTORY_DEBUG: Error in /manual_metrics_test: {e}")
            return f"Error generating metrics in /manual_metrics_test: {e}", 500

    metrics.init_app(app)
    print(f"FACTORY_DEBUG: PrometheusMetrics.init_app(app) called on {app}.")
    metrics.info('my_app_info_factory', 'System Load App - Factory', version='1.0.1-factory-debug')
    print(f"FACTORY_DEBUG: app.url_map after metrics.init_app: {str(app.url_map)}")
